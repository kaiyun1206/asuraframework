...
public class LeastActive {
    ...
    public LeastActive(List<IBalanceNode> nodes) {
        ...
        for (IBalanceNode node : nodes) {
            activeMap.put(node.getUniqueNodeName(), new AtomicInteger(0));
        }
    }
    ...
    public IBalanceNode select() {
        ...
        for (int i = 0; i < nodes.size(); i++) {
            ...
            int active = activeMap.get(node.getUniqueNodeName()).get();
            // 如果比当前活跃数更小
        ...
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info("select node: {}", selectNode.getUniqueNodeName());
        }
        activeMap.get(selectNode.getUniqueNodeName()).incrementAndGet();
        return selectNode;
    }
}
