...
public class  JackJsonFormat implements IJson {
    ...
    public static JackJsonFormat getInstance() {
        if (Check.isNull(jackJsonFormat)) {
            synchronized (JackJsonFormat.class) {
                if (Check.isNull(jackJsonFormat)) {
     ...
     */
    @Override
    public <T> List<T> getPathObjectList(@Nonnull String json, @Nonnull Class<T> t, @Nullable String... paths) {
        JsonNode jsonNode = parseJsonNode(json, paths);
        if (Check.isNull(jsonNode) || jsonNode.isNull()) {
            return null;
        }
